...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File scripts\run-course.ps1 -CourseId 001_cicd
...
if ($exitCode -eq 0) {
    Write-Log "完了しました。ログ: $LogPath"
} else {
    Write-Log "失敗したレッスンがあります（終了コード $exitCode）。ログ: $LogPath"
}
